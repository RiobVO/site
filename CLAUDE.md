...
**Позиционирование (2026-06-04):** мета главной (`title`/`og`/`twitter`) ведёт с
**«Цифровые продукты под ключ»**, НЕ «Backend-разработка» — Vire продаёт законченные продукты,
не «бэкенд-кусок» (реальный микс кейсов: боты, скоринг, платформы). Описание — клиентская выгода
«превращаю бизнес-процессы в работающий софт», без серверного жаргона (Clean Architecture и т.п.
для широкого/бизнес-клиента не читаются). НЕ возвращать «backend» в заголовки/мету. Слово «backend»
легитимно ТОЛЬКО в личном контексте (Элёр — backend-инженер по ремеслу): блок «Кто я», `Person.jobTitle`.
Hero-слоган («Проектирую. Строю. Запускаю.») и так универсален — не трогать.

### URL-схема — clean URLs (2026-06-04)

Cloudflare Assets раздаёт страницы БЕЗ `.html` (`/pricing`, `/case/credit-assistant`); запрос
`*.html` → 307 → clean URL. Поэтому в коде ВСЕ ссылки/`canonical`/`og:url`/`sitemap` — без `.html`.
Межстраничные ссылки **абсолютные от корня**: `/`, `/#work`, `/#principles`, `/pricing`,
`/pricing#02`, `/pricing#03`, `/case/<slug>`. НЕ добавлять `.html` обратно (вернёт лишний 307
и рассинхрон canonical). Главная и так была чистой (`https://vire.dev/`).

## Run locally

```bash
npx wrangler dev        # повторяет прод: clean URL (/pricing без .html) + форма /api/contact
# open http://localhost:8787
```

`python -m http.server 8000` тоже работает, НО не эмулирует clean-URL рерайт Cloudflare —
ссылки вида `/pricing` отдадут 404 (файла без расширения нет). Для просмотра навигации
целиком нужен `wrangler dev`; либо открывать конкретные `*.html` напрямую.

## Pages

> Live pages: **index.html · pricing.html · 404.html · case/\*.html** (7 cases). There is **NO resume page** (cv.html removed) and **NO audit landing** (audit.html removed) — both deleted by client decision, do NOT recreate either (details in the ⚠️ notes below).
...
- **pricing.html** — Pricing: three tiers (Consultation, Development, Audit). `§ 02` has `id="02"`, `§ 03 · Аудит` has `id="03"` (deep-link targets). Audit is sold here as a tier — NO separate landing page anymore. **§ 02 has TWO payment doors (2026-06-04):** fixed-price «от $2K / проект» (defined scope — bots/integrations) + «$4K/мес» retainer (large, evolving products) — deliberately covers BOTH small and large clients (matches the budget chips starting at $1–3K). Do NOT collapse back to retainer-only (it orphaned small-project leads). One-liner is «продукт целиком», NOT «весь бэкенд» (positioning). All three tier CTAs point to `#contact` form. **Value ladder (must hold):** Консультация $800–$1.5K (plan, «зачитывается в стоимость разработки») < Аудит $1K–$2.5K < Разработка от $2K. The plan MUST stay cheaper than the product — do NOT raise consultation back to $1.5–3.5K (that made the plan look pricier than the build). JSON-LD offer + priceRange ($800–$4K) track these numbers.
- **404.html** — Error page
- **case/*.html** — 7 case studies: credit-assistant, bando, container-bot, serviceflow, support-bot, manicure-bot, pekarna-bot. Each ends with a `.case-cta` block (Telegram + contextual link: bots/platforms → `pricing.html#02`, code-quality cases → `pricing.html#03`). Case-nav ring: credit-assistant → bando → container-bot → serviceflow → support-bot → manicure-bot → pekarna-bot.

**bando case (добавлен 2026-07-23):** `case/bando.html` — платформа подготовки к IELTS для клиента **bando.study** (бренд публичен — владелец разрешил называть и ссылаться). Факты только из репо E:\IELTS: 5,5 недель, ~75k строк TS, 1814 unit-тестов (прогон vitest 2026-07-23; 132 файла) + Playwright e2e, 37 таблиц, Next.js 15/React 19/Supabase/Drizzle/Vercel, оплаты Payme/Click/Uzum (ждут ключей), Writing/Speaking с AI (Gemini) за фиче-флагом. **Статус честный: Pre-launch** — публичного запуска НЕ было; НЕ писать «в проде»/«есть пользователи», в счётчик «Четыре запущены» bando НЕ входит (workstation: статус `prelaunch`, бейдж Pre-launch, статус-бар «4 live · 6 projects»; заголовок секции — «Ещё шесть проектов. Четыре запущены.»). 4 фигуры: Рис. 1 лендинг (`img/bando.*`), Рис. 2 экзамен-плеер Reading (`img/bando-player.*`), Рис. 3 очередь разбора ошибок (`img/bando-mistakes.*`), Рис. 4 CSS-схема layout. Скрины изнутри сняты 2026-07-23 через сессию владельца (Playwright, persistent-профиль); при обновлении — только с его логином, тестовые аккаунты в прод-базе клиента не создавать. `og/bando.png` — фирменная карточка в стиле остальных og (генерится ad hoc: HTML-шаблон → Playwright 1200×630). Отзыва клиента НЕТ (реальных слов нет — не выдумывать). WS_PROJECTS: поле `case` теперь у всех clean-URL (`/case/<slug>`, без `.html`).

**⚠️ audit.html DELETED** (client decision): the dedicated audit landing was removed — the audit service lives only as `§ 03` tier on pricing.html. All "Запросить аудит"/"Аудит кода" CTAs point to `pricing.html#03`. Do NOT recreate audit.html. The "Аудит" nav item was also removed.

**⚠️ Переименование (2026-06-04):** `services.html` → **`pricing.html`** (URL `/pricing`). Пункт меню — **«Цены»/«Pricing»** (не «Услуги»). Порядок nav — **Работы · Принципы · Цены** (ценность до цены, воронка). Старый `/services` + `/services.html` → **301 на `/pricing`** (redirect в `src/index.js`). Deep-links — `pricing.html#01/#02/#03`. og-картинка пока физически `og/services.png` (имя файла, на пользователя не влияет; перегенерить при желании).
...
- **Workstation CLI browser** (`.workstation`): file tree left, project detail right. JS-generated from `WS_PROJECTS` array. Cascade animation on project switch. **3 фичи (одобрены владельцем 2026-07-23, углубление CLI-метафоры):** (1) результат проекта — git-diff строки `- было / + стало` (`.ws-diff`, поля `minus_*`/`plus_*` в WS_PROJECTS, «было» взято из § 01 кейсов — не выдумывать; красный токен `--diff-minus` в base.css, AA в обеих темах); (2) псевдо-файлы у активного проекта в дереве (`.ws-files`: `case.md`→кейс, `stack.json`/`metrics.log`→outline-pulse соответствующего блока, стрелки дерева файлы пропускают, Tab доходит); (3) команда-эхо `$ open <name>/` печатается в панели (`.ws-echo`, language-neutral, min-height против прыжка layout). Всё гейтится reduced-motion; дифф без анимации.
...
Possible 6th: Ихтиёр about credit-assistant as accountant — awaiting his real words. Reviews were on pricing.html too but removed (client decision — no duplication).
...
Форма на **pricing.html** (`.cform` — карточка с шапкой-статусом; бюджет выбирается чипами `.bchip`; `$`-лейблы) отправляет заявку через Cloudflare Worker владельцу в Telegram. **На index.html формы НЕТ** (была — убрана по решению владельца, контакт там = кнопки Telegram/email/аудит).
